...
class TNTSynonymsGetter(TNTTaxaGetter):
	def __init__(self, tntsource, tnt_dbname, tntsourceid, tnt_projectid):
		...
		logger.info("Transfer Synonyms from TNT Source {0} {1} {2}".format(tntsource['name'], tnt_dbname, tnt_projectid))
	...
	def getPageQuery(self):
		...
		s.[SynNameID] AS [SourceAcceptedTaxonID],
		t.[canonicalName],
		t.[scientificNameAuthorShip],
		t.[taxonRank]
		FROM [{2}] t
		INNER JOIN [DiversityTaxonNames_{3}].[dbo].[TaxonSynonymy] s
		ON(t.[SourceTaxonID] = s.[NameID])
		WHERE t.[rownumber] BETWEEN ? AND ?""".format(self.tntsourceid, self.tnt_projectid, self.temptable, self.tnt_dbname)
		return query
