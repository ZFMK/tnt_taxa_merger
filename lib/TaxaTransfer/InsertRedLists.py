...
class InsertRedLists(InsertTaxa):
	...
	def copyRedList(self):
		...
		while self.taxa is not None:
			...
			query = """
			INSERT INTO `TaxaRedListTempTable`
			(`TaxonomySourceID`, `SourceTaxonID`, `SourceProjectID`, `term`, `value`, `reference`)
			VALUES {0}
